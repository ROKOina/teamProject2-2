...
void PBRShader::Begin(ID3D11DeviceContext* dc,const RenderContext& rc)
{
	...
	dc->UpdateSubresource(shadowMapConstantBuffer.Get(), 0, 0, &cbShadowMap, 0, 0);
	

	//	テクスチャ設定
	ID3D11ShaderResourceView* srvs[] =
	{
		rc.shadowmapData.shadowMap[0],
	};
	dc->PSSetShaderResources(4, ARRAYSIZE(srvs), srvs);
}

void PBRShader::Draw(ID3D11DeviceContext* dc, RenderContext& rc, const Model* model)
{
	...
	for (const ModelResource::Mesh& mesh : resource->GetMeshes())
	{
		...
		for (const ModelResource::Subset& subset : mesh.subsets)
		{
			...
			cbsubset.adjustMetalness = subset.material->pbr.adjustMetalness;
			cbsubset.adjustSmoothness = subset.material->pbr.adjustSmoothness;
			...
			ID3D11ShaderResourceView* srvs[] =
			{
		        ...
		        subset.material->metallic_smoothness_map.Get(),
				subset.material->ambient_occlusion_map.Get(),
			};
			dc->PSSetShaderResources(0, ARRAYSIZE(srvs), srvs);
			dc->PSSetSamplers(0, 1, shadowMapSamplerState.GetAddressOf());
...
// 描画終了
void PBRShader::End(ID3D11DeviceContext* dc,const RenderContext& rc)
{
