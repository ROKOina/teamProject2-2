...
// 半球ライト情報
struct HemisphereLightData
{
    float4 skyVector; // 空方向
    float4 skyColor; // 空色
...
//--------------------------------------------
// normal				: 法線(正規化済み)
// hemisphereLightData	: 半球ライト情報
// 返す値				: 入射光
float3 CalcHemisphereLight(float3 normal, HemisphereLightData hemisphereLightData)
{
    float factor = dot(hemisphereLightData.skyVector.xyz, normal) * 0.5f + 0.5f;
    return lerp(hemisphereLightData.groundColor, hemisphereLightData.skyColor, factor);
